...
public class SwipeLayout2 extends FrameLayout {
    ...
    public enum ShowMode {
        //两种模式   一种是 LayDown 躺在下面  底部布局一直不动  滑动上层布局显示出来底部布局
        //         第二种是  拉出来的布局 pullOut 拉动上层布局  底部布局跟着往上滑动 再慢慢显示
        LayDown,
        PullOut
    }
    ...
    public interface SwipeListener {
        void onStartOpen(SwipeLayout2 layout);

        void onOpen(SwipeLayout2 layout);

        void onStartClose(SwipeLayout2 layout);

        void onClose(SwipeLayout2 layout);

        void onUpdate(SwipeLayout2 layout, int leftOffset, int topOffset);

        void onHandRelease(SwipeLayout2 layout, float xvel, float yvel);
    }
     ...
     * {@link SwipeLayout2.OnRevealListener}
     *
     ...
     * @param l        the {@link SwipeLayout2.OnRevealListener}
     */
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        @Override
        public boolean tryCaptureView(View child, int pointerId) { // 当捕获的view  为 表面的view  或者为底部的view 才能实现拖动效果
    ...
    void layoutPullOut() {
        View surfaceView = getSurfaceView();
        Rect surfaceRect = mViewBoundCache.get(surfaceView);//从缓存中取出View的Rect
    ...
    void layoutLayDown() {
        ...
        if (surfaceView != null) {
            surfaceView.layout(surfaceRect.left, surfaceRect.top, surfaceRect.right, surfaceRect.bottom);
            bringChildToFront(surfaceView);// 使用bringChildToFront()将view显示在屏幕最前方  把该视图置于其他所有子视图之上，如在FrameLayout中切换被叠放的视图
        }
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            try {
                ...
                handled = (boolean) m.invoke(view, SwipeLayout2.this, p, vId);

            } catch (Exception e) {
                e.printStackTrace();

                if (view.getOnItemLongClickListener() != null) {
                    handled = view.getOnItemLongClickListener().onItemLongClick(view, SwipeLayout2.this, p, vId);
                }
                if (handled) {
     ...
     * {@link SwipeLayout2.ShowMode}.PullOut and
     * {@link SwipeLayout2.ShowMode}.LayDown.
     *
     * @param mode
     */
    ...
    public View getCurrentBottomView() {
        List<View> bottoms = getBottomViews();
        // mCurrentDragEdge.ordinal()   返回枚举对象在枚举类中的序号。从0开始。
     ...
     * get the open status.
     *
     * @return {@link SwipeLayout2.Status} Open , Close or
     * Middle.
     */
    ...
    public interface DoubleClickListener {
        void onDoubleClick(SwipeLayout2 layout, boolean surface);
    }
